...
for i in range(1, 21):
    ...
    for n_features in feature_range:
        ...
        cv_rmse = -cross_val_score(catboost_model, X_train[selected_features], y_train, scoring='neg_root_mean_squared_error', cv=kf).mean()
...
# 可以选择将结果保存为CSV文件
all_results_df.to_csv('ts-rfecatboost.csv', index=False)
