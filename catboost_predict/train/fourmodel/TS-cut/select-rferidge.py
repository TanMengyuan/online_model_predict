...
for i in range(1, 21):
    ...
    for n_features in feature_range:
        ...
        cv_rmse = -cross_val_score(ridge_model, X_train[selected_features], y_train, scoring='neg_root_mean_squared_error', cv=kf).mean()
        cv_r2 = cross_val_score(ridge_model, X_train[selected_features], y_train, scoring='r2', cv=kf).mean()
...
# 可以选择将结果保存为CSV文件
all_results_df.to_csv('ts-rferidge.csv', index=False)
